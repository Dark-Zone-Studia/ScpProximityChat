    ...
    internal class EventHandlers
    {
        ...
        private void OnVoiceChatting(VoiceChattingEventArgs ev)
        {
            ...
            if (_toggledPlayers.TryGetValue(player, out SpeakerToy speaker))
            {
                ...
                {
                    if (!(target.Role is IVoiceRole voiceRole) || voiceRole.VoiceModule.ValidateReceive(player.ReferenceHub, VoiceChatChannel.Proximity) == VoiceChatChannel.None)
                        continue;
        ...
        private void OnChangingRole(ChangingRoleEventArgs ev)
        {
            ...
            if (_config.ScpRoles.Contains(ev.NewRole))
            {
                ...
                switch (message.Type)
                {
                    case ScpProximityChat.Enums.MessageType.Broadcast when message.Show:
                        player.Broadcast(message.Duration, message.Content);
                        break;
                    case ScpProximityChat.Enums.MessageType.Hint when message.Show:
        ...
        private void OnSettingValueReceived(ReferenceHub hub, ServerSpecificSettingBase settingBase)
        {
            ...
            if (player == null || !_config.ScpRoles.Contains(player.Role.Type))
                return;
